...
class APIModule:
    ...
    def search_disease(self, disease_name):
        ...
        if response.status_code == 200:
            ...
            print(
                f"Error searching diseases for {disease_name}: {response.status_code}"
            )
            return []
    ...
    def select_closest_disease(self, disease_name, search_results):
        ...
        if closest_match is not None:
            ...
            for entry in search_results:
                if entry["dbName"] == closest_name and (
                    not find_orpha or entry["db"] == "ORPHA"
                ):
    ...
    def get_disease_info(self, disease_id):
        ...
        if response.status_code == 200:
            ...
            print(
                f"Error fetching disease info for {disease_id}: {response.status_code}"
            )
            return None
    ...
    def make_api_calls(self):
        ...
        incorrect_diseases = symptoms_df.apply(
            lambda row: [
                str(row[f"option_{answer}"])
                for answer in range(1, 6)
                if answer != row["correct_answer"]
            ],
            axis=1,
        ).apply(lambda lst: [string.replace(".", "") for string in lst])

        A = correct_disease.apply(self.get_disease_symptoms)
        B = incorrect_diseases.apply(
            lambda row: [
                symptom
                for disease in row
                for symptom in self.get_disease_symptoms(disease)
            ]
        )
        ...
        D = symptoms_df.apply(
            lambda row: list(
                set(tuple(row["A"]))
                .union(set(tuple(row["B"])))
                .difference(set(row["C"]))
            ),
            axis=1,
        )
        symptoms_df["D"] = D
